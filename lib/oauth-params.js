class OAuthParams{
  ...
  //TODO:reqのparamtersをそのまま読むようにしないと、サービスごとの差分を吸収できない
  generate(req){
    let oauth_params = Object.assign({}, req.parameters);
    oauth_params.oauth_signature_method = oauth_params.oauth_signature_method || "HMAC-SHA1"; 
    oauth_params.oauth_timestamp = this._getTimestamp();
    oauth_params.oauth_nonce = this._getNonce();
    oauth_params.oauth_signature = this._generateSignature(req, oauth_params);
    return params;
  }
  ...
  * */
  _generateSignature(req, oauth_params){
    ...
    if (oauth_params['signature_method'] == 'PLAINTEXT'){
      return secretKey;
    }

    if (oauth_params['signature_method'] == 'HMAC-SHA1'){
      let esc_method = this._oauthEscape(req.method);
      let esc_url = this._oauthEscape(req.url);
      let esc_norm_params = this._oauthEscape(this._normalize(oauth_params));

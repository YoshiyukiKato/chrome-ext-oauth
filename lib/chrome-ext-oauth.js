  ...
  authorizeAsync(){
    ...
    return new Promise((resolve, reject) => {
      if(this.params.url_authorize && this.params.url_authorize_callback){  
        ...
        let key = this.synonyms.request_token;
        let val = this.tokens.request_token;
        let url = `${baseurl}?${key}=${val}`;
...
function handleAuthorizeCallback(tabId, changeInfo, tab){
  ...
  if(urlobj.baseurl === this.params.url_authorize_callback){
    if(urlobj.params[this.synonyms.oauth_verifier]
    ){
      let oauth_verifier = urlobj.params[this.keys.oauth_verifier];
      this.setToken({ oauth_verifier: oauth_verifier });
      chrome.runtime.sendMessage({ type:"authorize", status:"success" });
    }else{
      chrome.runtime.sendMessage({ type:"authorize", status:"error" });
    }
  }else{
    //nothing to do 
  }
